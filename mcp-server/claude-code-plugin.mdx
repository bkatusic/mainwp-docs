...
Claude Code suppresses a plugin MCP server it considers a duplicate of a manually configured one, matching on the server name or on an identical command and args. With a hand-added `mainwp` entry present, the plugin's server never starts and the manual entry silently wins; the plugin still contributes its commands and skill.
