...
The toolbar badge shows two numbers separated by a hyphen, for example `5-2`. The first number is the total pending updates across plugins, themes, WordPress core, and translations. The second number is the count of Non-MainWP changes. Each number is capped at `99`.

When a Time Tracker timer is running, the badge switches to live elapsed time. Update counts return to the badge when the timer stops.
