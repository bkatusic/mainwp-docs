...
Most monitoring details appear inside the extension popup. The toolbar badge appears only when there is something to show and after the extension has run an update-count sync.

When an update-count sync finds pending updates or Non-MainWP changes, the badge can show two numbers separated by a hyphen, for example `5-2`. The first number is the filtered total for selected update types across plugins, themes, WordPress core, and translations. The second number is the count of Non-MainWP changes. Each number is capped at `99`, and the badge is blank when both counts are zero.
