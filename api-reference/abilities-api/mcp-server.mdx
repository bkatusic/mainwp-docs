...
The MCP server exposes around 60 tools. The exact count varies by MainWP Dashboard version. Tool names omit the primary namespace, which is `mainwp` by default, and replace hyphens with underscores. For example, `mainwp/list-sites-v1` becomes `list_sites_v1`. Tools from additional namespaces use the `{namespace}__{tool}` format, such as `acme__do_thing_v1`.
...
The `mainwp://site/{id}` resource provides information about an individual site. Keep `mainwp` in `abilityNamespaces` because this resource and site ID prompt completions call `mainwp/get-site-v1` and `mainwp/list-sites-v1` directly.
...
| `MAINWP_ABILITY_NAMESPACES` | No | `mainwp` | Comma-separated allowlist of ability namespaces |
...
Configuration loads from `./settings.json` or `~/.config/mainwp-mcp/settings.json`, in that order. Environment variables override file settings. In `settings.json`, use camelCase keys such as `apiToken`, `allowedTools`, `blockedTools`, `safeMode`, `schemaVerbosity`, `responseFormat`, `abilityNamespaces`, `rateLimit`, `requestTimeout`, `maxResponseSize`, and `maxSessionData`. `responseFormat` supports `compact` or `pretty`. `abilityNamespaces` accepts an array of allowed namespaces and defaults to `["mainwp"]`; the first entry is the primary namespace. For bearer-token authentication, use `apiToken`, which maps to `MAINWP_TOKEN`. If both credential methods are present, the username plus Application Password pair takes precedence.
...
The server exposes around 60 tools, with the exact count depending on the MainWP Dashboard version. These tools take roughly 28,000 tokens in an AI client's context window. If you want a smaller MCP surface area, use compact schemas and tool filtering.
