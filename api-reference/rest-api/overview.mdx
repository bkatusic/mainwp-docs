...
Use this URL to import the API into Postman, generate client code, or feed your own tooling. It covers every v2 endpoint, including extension routes, which are marked with `x-requires-extension` since they respond only when the matching extension is installed on your Dashboard. The `info.version` field records the MainWP Dashboard version the specification was written against.
