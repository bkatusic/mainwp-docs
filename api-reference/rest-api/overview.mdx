...
- Consumer Key and Consumer Secret (legacy `/mainwp/v1`, existing keys only)
...
- How to enable and use v2 API keys
...
Click **Save Settings**, then copy the generated credentials and store them safely.

<Warning>
The API key (Bearer token) is shown once and cannot be revealed later.
</Warning>
</Step>
...
MainWP still supports legacy v1 endpoints (`/wp-json/mainwp/v1/`) for backward compatibility with existing keys.

Creating new v1 keys is no longer supported.
You will only see v1 keys in MainWP Dashboard if they were already created in the past.
