...
| Method | Path | Purpose | Key Params |
|---|---|---|---|
| GET | `/clients/fields` | List client fields | `search`, `include`, `exclude`, `page`, `per_page`, `pre_page` (deprecated) |
...
<Note>
For `GET /clients/fields`, use `per_page` to set the number of fields returned per page. The `pre_page` parameter remains available as a deprecated alias for backward compatibility.
</Note>
