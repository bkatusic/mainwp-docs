...
Remove the MainWP Child plugin from the WordPress plugins list so clients cannot see it. The hide toggle is in the same **White Label** section as **Plugin name**, **Plugin description**, **Plugin URI**, **Plugin author**, and **Author URI**, so you can hide the plugin and save your custom branding details at the same time.
...
In the **White Label** section, set **Visually Hide MainWP Child Plugin** to **YES**. If needed, also fill in **Plugin name**, **Plugin description**, **Plugin URI**, **Plugin author**, and **Author URI** before saving.
...
<Note>
Hiding the plugin does not disable the other branding fields. If you use reporting or other areas that still surface the child plugin details, set the branding fields as well so those views use your custom plugin information instead of the default MainWP Child details.
</Note>
