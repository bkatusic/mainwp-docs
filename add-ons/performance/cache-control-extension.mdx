...
For the Cloudflare cache purging to work, you must enter a valid Cloudflare API Email and Cloudflare API Key and enable "Use Cloudflare Cache API" setting. <img src="/images/add-ons/cache-control-cloudflare-settings.png" alt="Screenshot of Cloudflare API settings with Email and API Key fields" /> **NOTE** Since version 4.0.1, the Cache Control extension will handle cache purging of both Cloudflare and the supported caching plugin at the same time. Previously, if both caching solutions were present, only the caching plugin was supported.
